export default () => {
  ...
  async function saveSubscription(subscription) {
    //Push notification on: 'https://push-notifications-admin.surge.sh/'
    const url =
  ...
  document.querySelector(".subscribeBtn").addEventListener("click", (event) => {
    ...
    servicew.pushManager.getSubscription().then(async (subscription) => {
      if (subscription) {
        ...
        document.querySelector(".subscribeBtn").innerHTML = "Subscribe app";
      } else {
        try {
          ...
          event.srcElement.disabled = false;
          document.querySelector(".subscribeBtn").innerHTML = "Unsubscribe";
        } catch (error) {}
      }
    });
  });
